        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else v-for="(transaction, index) in transactions" :key="transaction.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-red-600 font-medium">-{{ transaction.jumlah }}</td>
...
export default {
  ...
  methods: {
    ...
    async fetchItems() {
      try {
        // Get all items for dropdown - use large per_page to get all items
    ...
    async saveTransaction(formData) {
      try {
        ...
        this.showNotification('success', 'Berhasil', 'Transaksi barang keluar berhasil ditambahkan')
        this.closeModal()
        await this.fetchTransactions()
